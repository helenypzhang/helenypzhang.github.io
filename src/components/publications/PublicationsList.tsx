...
export default function PublicationsList({ config, publications, embedded = false }: PublicationsListProps) {
    ...
    const groupedPublications = useMemo(() => [
        { key: 'journal', title: messages.publications.journalPapers },
        { key: 'conference', title: messages.publications.conferencePapers },
        { key: 'preprint', title: messages.publications.preprints },
    ].map(group => ({
        ...group,
        publications: filteredPublications.filter(publication => publication.type === group.key),
    })).filter(group => group.publications.length > 0), [filteredPublications, messages]);
                ...
                {filteredPublications.length === 0 ? (
                    ...
                    groupedPublications.map(group => (
                      <section key={group.key}>
                        <h2 className="mb-5 text-2xl font-serif font-bold text-primary">{group.title}</h2>
                            ...
                            <div className="flex flex-col md:flex-row gap-6">
                                {pub.preview && (
                                    <div className="w-full md:w-48 flex-shrink-0">
                                        <div className="aspect-[16/10] relative rounded-lg overflow-hidden bg-neutral-100 dark:bg-neutral-800">
                                            <Image
